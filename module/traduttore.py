...
from .statements.statements import while_statement, while_statement_end, if_tree, else_do_tree, endif_statement
...
def to_asm(row:str, n_line:int) -> int:
    ...
    if re.fullmatch(fast_add, row): # operation like a += 12 + b ...
        text += fast_assigment(row, n_line, 1)
        return 0


    if re.fullmatch(fast_sub, row): # operation like a -= 12 + b ...
        text += fast_assigment(row, n_line, 0)
        return 0
    ...
    if re.match(if_re, row):
        text += if_tree(row, n_line)
        return 0

    if re.match(else_do, row):
        text += else_do_tree(row, n_line)
        return 0

    if re.match(endif, row):
        text += endif_statement(row, n_line)
        return 0


    # funzioni classiche 
    if re.match(r"\s*fork\s*\(\s*\)\s*\n?", row):   # creazione di sottoprocessi tramite fork()
        text += "\n\tmov rax, 57 ; fork()\n"
        text += "\tsyscall\n"
        return 0
