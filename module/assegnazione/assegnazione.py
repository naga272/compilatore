...
def recognize_type_var(row:str, n_line:int) -> str:
    ...
    tokenize[KEY] = tokenize[KEY].strip()
...
def fast_assigment(row:str, n_line:int, SUB_OR_ADD:int) -> str:
    """
    ...
    if(SUB_OR_ADD == SUB):
        ...
        result = eval(tokenize[VALUE]) 
        #print(memory_register[tokenize[KEY]][1], rbp)
        assembly_result += f"\tsub qword [rbp - {rbp - memory_register[tokenize[KEY]][1]}], " + str(result)
